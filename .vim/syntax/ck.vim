...
" CHUCK extentions
syn keyword ckLibrary			Std Math RegEx
...
syn keyword ckType              dur time complex polar Shred UGen Event Object Chubgraph Chugen
...
syn keyword ckUgen				Gain Noise Impulse Step Phasor Osc SinOsc PulseOsc TriOsc SqrOsc SawOsc Halfrec Fullrect Zerox Delayp SndBuf SndBuf2 Pan2
syn keyword ckSDK               bandedWG blowbotl BlowHole Bowed Brass Clarinet Flute Mandolin ModalBar Moog Saxofony Shakers Sitar StifKarp VoicForm FM BeeThree FMVoices HevyMetl PercFlut Rhodey TubeBell Wurley Delay DelayA DelayL Echo Envelope ADSR BiQuad Filter FilterBasic OnePole TwoPole OneZero TwoZero PoleZero LPF HPF BPF BRF JCRev NRev PRCRev ResonZ Chorus Modulate PitShift SubNoise WvIn WvOut WaveLoop LiSa
...
syn keyword ckCommunication     MidiIn MidiOut MidiFileIn MidiRW MidiMsg OscSend OscRcv Hid IO FileIO SerialIO StringTokenizer chout cherr ConsoleInput
syn keyword ckConstants 		PI TWO_PI E I INFINITY RANDOM_MAX INT_MAX FLOAT_MAX FLOAT_MIN_MAG
syn match   ckOperator          "\s*\(+\|-\|/\|*\|\s*\)=\(>\|<\|\^\)\s*" 
syn match	ckOperator			"<="
syn match	ckOperator			"<<"
syn match	ckOperator			"[\+\-\*\%]"
syn match	ckOperator			"[/*]\@<!/[/*]\@!"
syn match 	ckStatement			"<<<"
syn match 	ckStatement			">>>"
...
"  HiLink ckCommunication        Special
  HiLink ckNetwork              ckCommunication
...
"  HiLink ckUgen                 ckType
  ...
  HiLink ckShreds				Special
  HiLink ckInheritance			Special
  HiLink ckBoolean				Number
  HiLink ckIO					ckUgen
